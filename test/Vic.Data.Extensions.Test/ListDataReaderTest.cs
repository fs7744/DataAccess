    ...
    public class ListDataReaderTest
    {
        ...
        public void TestIntIndexer()
        {
            ...
            while (reader.Read())
            {
                ...
                foreach (var p in ps)
                {
                    var v = p.GetMethod.Invoke(st, new object[0]);
                    Assert.Equal(v == null ? DBNull.Value : v, reader[index]);
                    index++;
                }
            }
        }
        ...
        public async void TestNameIndexer()
        {
            ...
            foreach (var st in _Students)
            {
                ...
                {
                    var v = p.GetMethod.Invoke(st, new object[0]);
                    Assert.Equal(v ?? DBNull.Value, reader[p.Name]);
                }
            }
        }
        ...
        public async void TestGetValue()
        {
            ...
            foreach (var st in _Students)
            {
                ...
                {
                    var v = p.GetMethod.Invoke(st, new object[0]);
                    Assert.Equal(v == null ? DBNull.Value : v, reader.GetValue(index++));
                }
            }
        }
        ...
        public async void TestGetString()
        {
            ...
            foreach (var st in _Students)
            {
                ...
                foreach (var p in ps)
                {
                    ...
                    {
                        Assert.Equal(DBNull.Value, reader.GetValue(index));
                    }
                    else
                    {
